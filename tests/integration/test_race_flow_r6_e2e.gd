...
## (docs/art/gates/2026-08-02-{cortex,coco,ripper-roo}/gate-record.md) are
## now operator-accepted, same shape as papu's own earlier Task 5 flip.
const CORTEX_CHARACTER_SCENE_PATH := "res://assets/models/characters/SK_cortex.glb"
const COCO_CHARACTER_SCENE_PATH := "res://assets/models/characters/SK_coco.glb"
const RIPPER_ROO_CHARACTER_SCENE_PATH := "res://assets/models/characters/SK_ripper_roo.glb"
...
func test_r6_seeded_sanity_shores_full_flow_fx_items_characters_apex_ai_and_standings() -> void:
	...
	# Task 5 (papu) + R8 gate flip 2026-08-02 (cortex/coco/ripper_roo): AI
	# fill walks DriverRegistry.entries() in its own fixed ROSTER ORDER
	# minus the player's pick (_ai_fill_driver_ids()'s own doc) -- with the
	# player defaulting to crash (this fixture never picks), that order is
	# papu, cortex, coco, ripper_roo, lab_assistant, so opponent_index 0-4
	# map 1:1 onto those five ids in order. Every one of them now ships a
	# real, operator-accepted character_scene_path (papu's own Task 5 flip;
	# cortex/coco/ripper_roo's own R8 gate flip) or, for lab_assistant, its
	# own always-real path -- so no AI kart in this run mounts the shared
	# fallback mesh any more; each rides its own distinct real model.
	const _EXPECTED_AI_SCENE_PATHS := [
		PAPU_SEATED_CHARACTER_SCENE_PATH,
		CORTEX_CHARACTER_SCENE_PATH,
		COCO_CHARACTER_SCENE_PATH,
		RIPPER_ROO_CHARACTER_SCENE_PATH,
	...
	for opponent_index in range(opponent_count):
		...
		if ai_mount != null:
			assert_eq(
				...
				_EXPECTED_AI_SCENE_PATHS[opponent_index],
				"AI kart %d must ride its own real registered model" % opponent_index
			)
	...
	#
	# R8 gate flip 2026-08-02: cortex/coco/ripper_roo are the last three
	# drivers that used to be fallback-active (Tasks 6-8's own likeness
	# gates, docs/art/gates/2026-08-02-{cortex,coco,ripper-roo}/gate-
	# record.md, all operator-accepted) -- papu flipped earlier at Task 5.
	# With all six roster ids now shipping real gated scenes, DriverRegistry.
	# character_scene() never reaches _fallback_scene() for anyone in this
	# run, so the CTR R8 Task 2/fix-round-2 exclusion bucket this block used
	# to carry (matching expected-fallback driver ids out of get_errors())
	# has nothing left to exclude -- reverted to this assertion's own
	# original pre-Task-2 strict shape (CTR R6 Task 6): a bare zero-errors
	# check. A regression that made any driver fall back again -- expected
	# or not -- now fails this assertion directly and loudly, which is a
	# TIGHTER bound than the bucketed version, not a weaker one.
	# ------------------------------------------------------------------
	assert_eq(
		get_errors().size(),
		0,
