...
## Fix-wave MEDIUM-5: solo time trial restored -- the level list's own
## RacingTimeTrial/RacingSanityShores buttons above now launch the AI-
## populated RACE scenes (their button TEXT was relabeled "RACE — ..." to
## match, see level_list_overlay.tscn; their own NODE PATHS/root scene node
## names are untouched -- the two tests above keep passing unmodified), so
## the spec's "time trial ships as race-minus-AI" promise needs its OWN,
## new entries. Same real click -> real GameRoot dispatch shape as every
## racing test above, landing on scenes/racing/race_time_trial_solo.tscn
## instead (root node "RaceTimeTrialSolo").
func test_real_level_list_opens_racing_time_trial_solo() -> void:
	var root := _instantiate_main()
	if root == null:
		return
	await wait_process_frames(1)
...
func test_real_level_list_opens_racing_sanity_shores_time_trial_solo() -> void:
	var root := _instantiate_main()
	if root == null:
		return
	await wait_process_frames(1)
	...
	var overlay := root.get_node("UI/LevelListOverlay")
	var racing_button := overlay.get_node(
		"SafeArea/Center/Panel/Margin/Rows/RacingSanityShoresTimeTrial"
	) as Button
	assert_true(
		racing_button.visible,
		"debug builds must expose the solo time trial entry through the real list"
