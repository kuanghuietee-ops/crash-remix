...
func test_coyote_jump_uses_ground_jump_inside_exact_window() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 1.0, true, 0.0, buffer, _move, _input)
	fsm.call("step", 1.01, false, 0.0, buffer, _move, _input)
	_press(buffer, &"jump", 1.14)

	var decision: RefCounted = fsm.call(
		"step", 1.14, false, 0.0, buffer, _move, _input
	)
...
func test_coyote_boundary_stays_inclusive_at_a_later_clock_origin() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 10.0, true, 0.0, buffer, _move, _input)
	fsm.call("step", 10.01, false, 0.0, buffer, _move, _input)
	_press(buffer, &"jump", 10.14)

	var decision: RefCounted = fsm.call(
		"step", 10.14, false, 0.0, buffer, _move, _input
	)
...
func test_jump_just_outside_coyote_window_uses_double_jump() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 2.0, true, 0.0, buffer, _move, _input)
	fsm.call("step", 2.01, false, 0.0, buffer, _move, _input)
	_press(buffer, &"jump", 2.141)

	var decision: RefCounted = fsm.call(
		"step", 2.141, false, 0.0, buffer, _move, _input
	)
...
func test_buffered_jump_fires_on_landing_after_both_jumps_are_spent() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 3.0, true, 0.0, buffer, _move, _input)
	_press(buffer, &"jump", 3.01)
	fsm.call("step", 3.01, true, 0.0, buffer, _move, _input)
	_release(buffer, &"jump", 3.02)
	_press(buffer, &"jump", 3.2)
	fsm.call("step", 3.2, false, 0.0, buffer, _move, _input)
	_release(buffer, &"jump", 3.21)
	_press(buffer, &"jump", 4.0)
	fsm.call("step", 4.0, false, 0.0, buffer, _move, _input)

	var decision: RefCounted = fsm.call(
		"step", 4.119, true, 0.0, buffer, _move, _input
	)
...
func test_down_still_crouches_and_down_while_running_slides() -> void:
	...
	if crouch_fsm == null or crouch_buffer == null or slide_fsm == null or slide_buffer == null:
		return
	crouch_fsm.call("step", 5.0, true, 0.0, crouch_buffer, _move, _input)
	_press(crouch_buffer, &"down", 5.01)
	var crouch: RefCounted = crouch_fsm.call(
		"step", 5.01, true, 0.0, crouch_buffer, _move, _input
	)
	slide_fsm.call("step", 5.0, true, _move.get("run_speed_mps"), slide_buffer, _move, _input)
	_press(slide_buffer, &"down", 5.01)
	var slide: RefCounted = slide_fsm.call(
		"step", 5.01, true, _move.get("run_speed_mps"), slide_buffer, _move, _input
	)
...
func test_tiny_analog_drift_counts_as_still_for_crouch() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 5.0, true, 0.0, buffer, _move, _input)
	_press(buffer, &"down", 5.01)

	var decision: RefCounted = fsm.call(
		"step", 5.01, true, 0.5, buffer, _move, _input
	)
...
func test_jump_during_slide_becomes_slide_jump() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 6.0, true, _move.get("run_speed_mps"), buffer, _move, _input)
	_press(buffer, &"down", 6.01)
	fsm.call("step", 6.01, true, _move.get("run_speed_mps"), buffer, _move, _input)
	_press(buffer, &"jump", 6.1)

	var decision: RefCounted = fsm.call(
		"step", 6.1, true, _move.get("run_speed_mps"), buffer, _move, _input
	)
...
func test_same_frame_ground_jump_beats_new_down_press_without_zero_length_slide() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 6.25, true, _move.get("run_speed_mps"), buffer, _move, _input)
...
func test_crouch_jump_uses_the_section_4_2_high_jump() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 6.5, true, 0.0, buffer, _move, _input)
	_press(buffer, &"down", 6.51)
	fsm.call("step", 6.51, true, 0.0, buffer, _move, _input)
	_press(buffer, &"jump", 6.6)

	var decision: RefCounted = fsm.call(
		"step", 6.6, true, 0.0, buffer, _move, _input
	)
...
func test_down_in_air_starts_uncancellable_body_slam() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 7.0, false, 0.0, buffer, _move, _input)
	_press(buffer, &"down", 7.01)
	var slam: RefCounted = fsm.call(
		"step", 7.01, false, 0.0, buffer, _move, _input
	)
	_press(buffer, &"jump", 7.02)
	var attempted_cancel: RefCounted = fsm.call(
		"step", 7.02, false, 0.0, buffer, _move, _input
	)
...
func test_double_jump_wins_same_frame_and_discards_losing_slam() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 7.5, true, 0.0, buffer, _move, _input)
	_press(buffer, &"jump", 7.51)
	fsm.call("step", 7.51, true, 0.0, buffer, _move, _input)
	_release(buffer, &"jump", 7.52)
	_press(buffer, &"jump", 7.6)
	_press(buffer, &"down", 7.61)

	var jump_decision: RefCounted = fsm.call(
		"step", 7.62, false, 0.0, buffer, _move, _input
	)
	var slam_decision: RefCounted = fsm.call(
		"step", 7.63, false, 0.0, buffer, _move, _input
	)
...
func test_air_spin_is_once_per_airtime_and_a_late_second_press_fires_on_landing() -> void:
	...
	if fsm == null or buffer == null:
		return
	fsm.call("step", 8.0, false, 0.0, buffer, _move, _input)
	_press(buffer, &"spin", 8.01)
	var first_spin: RefCounted = fsm.call(
		"step", 8.01, false, 0.0, buffer, _move, _input
	)
	_release(buffer, &"spin", 8.02)
	fsm.call("step", 8.5, false, 0.0, buffer, _move, _input)
	_press(buffer, &"spin", 8.51)
	var blocked_spin: RefCounted = fsm.call(
		"step", 8.51, false, 0.0, buffer, _move, _input
	)
	var landing_spin: RefCounted = fsm.call(
		"step", 8.6, true, 0.0, buffer, _move, _input
	)
