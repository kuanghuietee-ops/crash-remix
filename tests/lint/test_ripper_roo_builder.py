"""R8 Task 8: boundary + determinism coverage for create_ripper_roo.py.

Mirrors tests/lint/test_coco_builder.py's own shape verbatim (that file's
own module doc explains the two-tier split this file repeats, and Task 7's
own report cites the same investigation this file's tolerance/scope choices
are copied from):

- RipperRooBuilderSourceTests are plain text-scan assertions (no Blender
  invocation) -- fast, always run.
- RipperRooBuilderExportTests builds the real GLB once and inspects the
  exported glTF document directly -- self-contained (no external buffer/
  image URIs, since this asset carries vertex colours only and never
  samples a texture).
- RipperRooBuilderDeterminismTests builds the real GLB twice and diffs the
  two outputs at the exact granularity test_cortex_builder.py's/test_coco_
  builder.py's own class proved out (the same shared character_asset_
  common.py pipeline, so the same non-determinism sources apply here).
  Skipped when the blender binary is not on this box.
"""
...
BUILDER_SCRIPT = REPO_ROOT / "scripts" / "blender" / "create_ripper_roo.py"
KART_CONTROLLER_SCRIPT = REPO_ROOT / "src" / "racing" / "kart" / "kart_controller.gd"
DRIVER_ENTRY_PATH = REPO_ROOT / "data" / "racing" / "drivers" / "ripper_roo.tres"
BLENDER_BIN = Path("/usr/bin/blender")

_WRAPPER_TEMPLATE = """
import sys
sys.path.insert(0, {scripts_dir!r})
import create_ripper_roo
from pathlib import Path

create_ripper_roo.main(
...
class RipperRooBuilderSourceTests(unittest.TestCase):
    ...
    def test_seated_action_name_matches_kart_controllers_seat_clip(self) -> None:
        # Same literal-name-match requirement test_cortex_builder.py's/
        # test_coco_builder.py's own test uses -- see kart_controller.gd's
        # SEAT_ANIMATION_CLIP doc ("same StringName, reused verbatim"). A
        # differently-named action here would silently fall through to
        # _apply_static_seat_pose(), which only overrides bones literally
        # named "DEF-*" -- Ripper Roo's own compact custom rig (build_rig()
        # below) does not have those, so he would mount standing, not
        # seated.
    ...
    def test_idle_action_name_is_literally_named(self) -> None:
        # Context's own naming convention: "idle action (A_ripper_roo_
        # idle)" -- the literal StringName the brief itself specifies,
        # matching A_cortex_idle/A_coco_idle's own per-character shape.
        builder_source = BUILDER_SCRIPT.read_text(encoding="utf-8")
        self.assertIn('IDLE = "A_ripper_roo_idle"', builder_source)
    ...
    def test_exports_into_the_characters_hero_budget_directory(self) -> None:
        ...
        # tres) Cortex's/Coco's own NEW builds were authored into -- see
    ...
    def test_reuses_character_asset_common_never_redefines_its_helpers(self) -> None:
        ...
        # same shared kit create_cortex.py/create_coco.py already use) is
        # that the rig/part/action primitives are IMPORTED, not
        # copy-pasted -- a fresh local reimplementation is exactly the kind
        # of drift this task's own STOP-rule (byte-deterministic, shared
        # pipeline) forbids.
        builder_source = BUILDER_SCRIPT.read_text(encoding="utf-8")
        for helper in (
            "add_cone_between",
    ...
    def test_no_props_beyond_the_straitjacket_read(self) -> None:
        # Plan-mandated YAGNI (same shape as Coco's own "no laptop" guard):
        # the brief names exactly four silhouette traits (straitjacket
        # torso, oversized feet, tongue-out head read, blue fur) -- no
        # stitches, bandages, chain, or other Ripper-Roo-canon iconography
        # was asked for. Scoped to build_parts()'s own body rather than the
        # whole file -- the module doc legitimately *discusses* these
        # excluded props in prose (to explain the YAGNI boundary), which
        # would otherwise self-trigger a whole-file substring ban.
        builder_source = BUILDER_SCRIPT.read_text(encoding="utf-8")
        ...
        body_lower = build_parts_match.group(0).lower()
        for excluded in ("stitch", "bandage", "chain", "staple"):
            self.assertNotIn(excluded, body_lower)
    ...
    def test_driver_entry_flip_matches_this_builders_own_authored_seat_fit(self) -> None:
        ...
        # flips ripper_roo.tres.
        #
        # R8 gate flip 2026-08-02: the operator accepted the face in
        # conversation (docs/art/gates/2026-08-02-ripper-roo/gate-
        # record.md's own "Result" line) and a separate flip commit set
        # ripper_roo.tres' own character_scene_path/seat_scale/seat_offset.
        # This test now pins that the flipped values are EXACTLY this
        # script's own already-authored GATE_SEAT_SCALE/GATE_SEAT_OFFSET_
        # GODOT constants -- parsed statically out of the real builder
        # source, not a hand-copied duplicate that could drift -- so a
        # flip that used the wrong numbers, or a future edit to one side
        # that forgot the other, both fail loudly here.
        builder_source = BUILDER_SCRIPT.read_text(encoding="utf-8")
        self.assertNotIn(
            "data/racing/drivers",
            builder_source,
        ...
        self.assertIsNotNone(entry_scale_match, "ripper_roo.tres must ship a plain seat_scale literal")
        self.assertIsNotNone(entry_offset_match, "ripper_roo.tres must ship a plain seat_offset literal")
    ...
    def test_seated_action_only_poses_the_allowlisted_bones(self) -> None:
        # Same static-source derivation test_cortex_builder.py's/test_coco_
        # builder.py's own identically-named test uses: parsed straight out
        # of the SEATED_POSE dict literal (the only place create_seated()
        # ever calls key_pose() with non-default rotations), not a
        # hand-maintained duplicate list that could drift out of sync with
        # the real behaviour. Ripper Roo's own seated action is a static
        # two-keyframe hold with no mid-cycle exception, same shape as
        # Cortex's/Coco's own (the bend angles differ -- deeper fold to
        # tuck the big feet onto the cowl -- but the allowlisted bone SET
        # is identical).
        builder_source = BUILDER_SCRIPT.read_text(encoding="utf-8")
        ...
        self.assertEqual(
            ...
            "allowlist and create_ripper_roo.py's own SEATED_POSE doc "
            "comment together, and re-confirm the likeness read if a "
            "face/torso bone was added",
        )
...
def _build_once(output_dir: Path) -> Path:
    export_path = output_dir / "SK_ripper_roo.glb"
    wrapper_path = output_dir / "run_builder.py"
    wrapper_path.write_text(
        _WRAPPER_TEMPLATE.format(
            scripts_dir=str(BUILDER_SCRIPT.parent),
            export_path=str(export_path),
            source_path=str(output_dir / "SK_ripper_roo.blend"),
            preview_path=str(output_dir / "SK_ripper_roo_preview.png"),
        ),
        encoding="utf-8",
    )
    ...
    if result.returncode != 0 or not export_path.is_file():
        raise RuntimeError(
            "create_ripper_roo builder failed:\n"
...
class RipperRooBuilderExportTests(unittest.TestCase):
    ...
    def test_glb_is_self_contained_with_no_external_references(self) -> None:
        with tempfile.TemporaryDirectory() as output_dir_name:
            ...
            self.assertEqual(
                document.get("images", []),
                [],
                "SK_ripper_roo.glb must carry no images/textures at all -- "
                "the likeness read is vertex-colour only, so any image "
...
## See test_cortex_builder.py's/test_coco_builder.py's own class doc for the
## full investigation behind this tolerance and scope -- the same shared
## character_asset_common.py pipeline (smart_project() UV packing, glTF
## triangle-list export order, per-vertex NORMAL summation order) applies
## unchanged here, none of it re-derived per builder.
...
class RipperRooBuilderDeterminismTests(unittest.TestCase):
