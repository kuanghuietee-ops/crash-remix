...
`LevelMeta`, then checks checkpoint spacing and explicit next-checkpoint links along
the authored Spine, collectible normal-mode crate totals and segment membership
(iron and relic-only time crates do not count toward the gem total), stable crate
IDs, required-jump camera depression, and relic-only time-crate placement.
