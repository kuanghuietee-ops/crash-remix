...
3. HARDER: a difficulty pass on ai.tres defaults — corner_speed_floor_ratio up (0.45→0.55), brake_margin_ratio up (1.12→1.18), apex_entry_lookahead tuned per measurement, personality_aggression_step up (0.15→0.2), rubber_band_drag_max_ratio down (0.12→0.08 — weaker catch-up drag on an AI leading the player, so it pulls away MORE instead of being reeled back in — harder, less mercy for a struggling player). Justify each with a measured lap-time delta (seeded solo-AI lap benchmark before/after in the report; target: clean-lap pace up ≥8% without new wedges).
