...
| D6 | P1 | hog_mount.gd:83-94 + player_controller.gd:149-151 | With null hog tuning, `mount_hog()` no-ops but HogMount sets `_mounted=true`, emits `mounted`, reparents visual | VERIFIED | `_set_mounted` only checks `has_method`, never the return |
...
| D9 | P2 | level_session.gd:682 | Hog mounts reset on plain checkpoint activation, not just respawn | VERIFIED | 5 callers of `_set_player_spawn`; :682 is `_on_checkpoint_reached`. Benign here: PROBE3 mount_progress=0.0, both checkpoints inside window |
| D10 | P2 | player_controller.gd:115; player_motor.gd:48,135; :557 | `hog_tuning` defaulted to null on 4 signatures; contradicts the repo's own no-silent-default test | VERIFIED | All 5 production call sites do pass `catalog.hog`; the default only protects tests |
| D11 | P2 | tests/gameplay/test_player_controller.gd:918 | Controller tests run with `_hog_tuning == null` | REPORTED | Agent-read; consistent with the optional param |
| D12 | P2 | scripts/verify_exported_tuning.sh:66 | LEVEL META grep asserts only n_sanity_beach | VERIFIED (low impact) | I ran the script: hog_wild.tres does print and the HOG WILD marker is asserted |
| D13 | P2 | test_level_scenes.gd:1103-1116 | Handoff test can pass with zero assertions | REPORTED | Agent-read |
| D14 | P2 | lint_level_authoring.py | `wumpa_total` is not linted at all | REPORTED | LevelMetaValues carries only crate_count + design_pace_mps |
